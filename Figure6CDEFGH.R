...
for (k in 1:length(environment)){
  ...
  wolbachia<-c(wolbachia,otu_table(dwr)[which(rownames(tax_table(dwr))==tchoice),hit])
...
dfred$wolbachia<-pmin(pmax(dfred$wolbachia, 1e-6), 1 - 1e-6)
...
#dfblue$wolbachia<-pmin(pmax(dfblue$wolbachia, 1e-6), 1 - 1e-6) #strain 1
dfblue$wolbachia<-pmin(pmax(dfblue$wolbachia, 1e-4), 1 - 1e-4)  #strain 3
...
#dfblue2$wolbachia<-pmin(pmax(dfblue2$wolbachia, 1e-5), 1 - 1e-5)
